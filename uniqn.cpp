...
Statistics::Ptr makeStatistics(int argc, char const* argv[]) {

  if (argc > 1) {
    std::string param(argv[1]);
    if (param == "-d") {
      // dump sorted dictionary, read from stdin
      Statistics stat(std::cin);
      Dictionary const& dict = stat.dictionary();
...
bool operator<(Result const& left, Result const& right) {
  return left.phrase.value() == right.phrase.value() ?
...
{
  Statistics::Ptr stat = makeStatistics(argc, argv);
  if (!stat)
    return 0;
